...
fn monkey_run(monkeys: &mut [Monkey], monkey_id: int, counter: &mut [int], m : int) {
    ...
    for &item in &monkey.items {
        ...
        let new = func(old).round() as int % m;
...
fn main() {
    ...
    let m = monkeys.iter().map(|x| x.test).product();

    let mut counter = vec![0; monkeys.len()];
    for _ in 0..10000 {
        for i in 0..monkeys.len() {
            monkey_run(&mut monkeys, i as int, &mut counter, m);
        }
    }
